...
while True:
    try:
        ...
        for (x, y, w, h) in faces:
            ...
            if label == "Unknown" and count_ids[label] % unknown_count_threshold == 0:
                # Save and send photo for every 20 unknown persons
                photo_path = save_photo(resized_frame)
                send_telegram_photo(photo_path)

            # Reset count ID for each label when the threshold is reached
            if count_ids[label] == unknown_count_threshold:
                count_ids[label] = 0
